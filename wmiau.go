...
var clientContainer *sqlstore.Container
...
func (s *server) startClient(userID int, textjid string, token string, subscriptions []string) {
	...
	// Create a new database connection for this client
	dbPath := filepath.Join(s.exPath, "dbdata", fmt.Sprintf("store_%d.db", userID))
	clientDB, err := sql.Open("sqlite", fmt.Sprintf("file:%s?_pragma=foreign_keys(1)&_busy_timeout=5000&_journal_mode=WAL", dbPath))
	if err != nil {
		log.Error().Err(err).Msg("Failed to open client database")
		return
	} else {
		log.Debug().Str("dbfile", dbPath).Msg("Database opened")
	}
	...
	if *waDebug != "" {
		...
		clientContainer = sqlstore.NewWithDB(clientDB, "sqlite", dbLog)
	} else {
		clientContainer = sqlstore.NewWithDB(clientDB, "sqlite", nil)
	}
	err = clientContainer.Upgrade()
	if err != nil {
		log.Error().Err(err).Msg("Failed to create client tables")
		return
	}
	...
	if textjid != "" {
		...
		deviceStore, err = clientContainer.GetDevice(jid)
		if err != nil {
			log.Error().Err(err).Msg("Failed to get device")
			return
		}
	} else {
		deviceStore = clientContainer.NewDevice()
	}
